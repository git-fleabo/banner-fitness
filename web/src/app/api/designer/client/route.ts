...
import { ptAssessments, ptClients, ptExercisePrescriptions, ptExercises, ptGoals, ptLocations, ptProgrammeEvents, ptProgrammeWeeks, ptProgrammes, ptSessions } from "@/lib/db/schema";
...
export async function GET(request: NextRequest) {
  ...
  const programmeHistory = await db.select({ id: ptProgrammes.id, name: ptProgrammes.name, goalSummary: ptProgrammes.goalSummary, status: ptProgrammes.status, version: ptProgrammes.version, updatedAt: ptProgrammes.updatedAt }).from(ptProgrammes).where(and(eq(ptProgrammes.clientId, client.id), eq(ptProgrammes.ownerProfileId, access.account.authUserId))).orderBy(desc(ptProgrammes.version));
  ...
  if (!programme) return NextResponse.json({ client, assessment: assessment ?? null, goal: goal ?? null, location: location ?? null, programmeHistory, programme: null });
  const events = await db.select({ id: ptProgrammeEvents.id, action: ptProgrammeEvents.action, details: ptProgrammeEvents.details, createdAt: ptProgrammeEvents.createdAt }).from(ptProgrammeEvents).where(eq(ptProgrammeEvents.programmeId, programme.id)).orderBy(desc(ptProgrammeEvents.createdAt));
  ...
  const sessionMap = new Map<string, { id: string; dayOfWeek: number; name: string; sessionType: string; durationMinutes: number; exercises: Array<{ name: string; pattern: string; prescription: string; target: string; equipment: string; intensityValue?: string; restSeconds?: number; tempo?: string; progressionRule?: string; method?: string; note?: string }> }>();
  for (const row of sessions) {
    ...
    if (row.exerciseName && row.pattern && row.sets) current.exercises.push({ name: row.exerciseName, pattern: row.pattern, prescription: row.repsMin ? `${row.sets} × ${row.repsMin}${row.repsMax && row.repsMax !== row.repsMin ? `–${row.repsMax}` : ""}` : `${row.sets} sets`, target: Array.isArray(row.target) ? row.target.join(" · ") : "", equipment: Array.isArray(row.equipment) ? row.equipment.join(", ") : "", intensityValue: row.intensityValue ?? undefined, restSeconds: row.restSeconds ?? undefined, tempo: row.tempo ?? undefined, progressionRule: row.progressionRule ?? undefined, method: row.technique ?? undefined, note: row.notes ?? undefined });
    sessionMap.set(row.id, current);
  }
  return NextResponse.json({ client, assessment: assessment ?? null, goal: goal ?? null, location: location ?? null, programmeHistory, programme: { ...programme, week: week ?? null, sessions: Array.from(sessionMap.values()), events } });
}
