...
export async function GET(request: NextRequest) {
  ...
  const results = await db.select({ id: ptWorkoutResults.id, scheduledDate: ptWorkoutResults.scheduledDate, sessionName: ptSessions.name, status: ptWorkoutResults.status, sessionRpe: ptWorkoutResults.sessionRpe, energy: ptWorkoutResults.energy, painReported: ptWorkoutResults.painReported, durationMinutes: ptWorkoutResults.durationMinutes, volumeLoadKg: ptWorkoutResults.volumeLoadKg, repetitionLoad: ptWorkoutResults.repetitionLoad, averageRpe: ptWorkoutResults.averageRpe, averageRir: ptWorkoutResults.averageRir, notes: ptWorkoutResults.notes }).from(ptWorkoutResults).leftJoin(ptSessions, eq(ptSessions.id, ptWorkoutResults.sessionId)).where(and(eq(ptWorkoutResults.clientId, client.id), eq(ptWorkoutResults.ownerProfileId, access.account.authUserId))).orderBy(desc(ptWorkoutResults.scheduledDate), desc(ptWorkoutResults.createdAt)).limit(100);
  const setRows = await db.select({ scheduledDate: ptWorkoutResults.scheduledDate, exerciseName: ptExercises.name, pattern: ptExercises.movementPattern, actualReps: ptWorkoutResultSets.actualReps, actualLoadKg: ptWorkoutResultSets.actualLoadKg, actualRpe: ptWorkoutResultSets.actualRpe, actualRir: ptWorkoutResultSets.actualRir }).from(ptWorkoutResultSets).innerJoin(ptWorkoutResults, eq(ptWorkoutResults.id, ptWorkoutResultSets.workoutResultId)).innerJoin(ptExercisePrescriptions, eq(ptExercisePrescriptions.id, ptWorkoutResultSets.prescriptionId)).innerJoin(ptExercises, eq(ptExercises.id, ptExercisePrescriptions.exerciseId)).where(and(eq(ptWorkoutResults.clientId, client.id), eq(ptWorkoutResults.ownerProfileId, access.account.authUserId))).orderBy(asc(ptWorkoutResults.scheduledDate));

  const exerciseMap = new Map<string, { exerciseName: string; pattern: string; sessions: Set<string>; totalReps: number; volumeLoadKg: number; bestLoadKg: number; latestLoadKg: number; latestDate: string | null }>();
  for (const row of setRows) {
    const current = exerciseMap.get(row.exerciseName) ?? { exerciseName: row.exerciseName, pattern: row.pattern, sessions: new Set<string>(), totalReps: 0, volumeLoadKg: 0, bestLoadKg: 0, latestLoadKg: 0, latestDate: null };
  ...
  return NextResponse.json({
    ...
    exercises: Array.from(exerciseMap.values()).map((exercise) => ({ ...exercise, sessions: exercise.sessions.size })),
