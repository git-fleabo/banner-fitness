...
export async function GET(request: Request) {
  ...
  const clients = await db.select({ id: ptClients.id, firstName: ptClients.firstName, lastName: ptClients.lastName, clientColour: ptClients.clientColour, status: ptClients.status, trainingExperience: ptClients.trainingExperience, sessionDurationMinutes: ptClients.sessionDurationMinutes, updatedAt: ptClients.updatedAt }).from(ptClients).where(designerOwnership(ptClients.ownerProfileId, owner)).orderBy(desc(ptClients.updatedAt)).limit(100);
  ...
  const requestedWeekOffset = Math.max(-12, Math.min(12, Number(new URL(request.url).searchParams.get("weekOffset") ?? 0) || 0));
  ...
  currentWeekMonday.setDate(currentWeekMonday.getDate() + requestedWeekOffset * 7);
  const schedule = baseSchedule.map((session) => {
    ...
    return { ...session, date, status: result ? (result.status || "completed") : (date < today ? "pending" : date === today ? "today" : "upcoming") };
  });
