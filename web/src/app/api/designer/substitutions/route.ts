...
export async function GET(request: NextRequest) {
  ...
  const db = getDb();
  const [client] = await db.select({ id: ptClients.id }).from(ptClients).where(and(eq(ptClients.id, clientId), eq(ptClients.ownerProfileId, access.account.authUserId))).limit(1);
  if (!client) return NextResponse.json({ error: "Client not found" }, { status: 404 });
  const [source] = await db.select({ id: ptExercises.id, name: ptExercises.name, pattern: ptExercises.movementPattern, target: ptExercises.primaryMuscles, difficulty: ptExercises.difficulty, equipment: ptExercises.equipment }).from(ptExercises).where(eq(ptExercises.id, exerciseId)).limit(1);
