...
export async function GET(request: Request) {
  ...
  const [client] = await db.select({ id: ptClients.id, firstName: ptClients.firstName, lastName: ptClients.lastName, sessionDurationMinutes: ptClients.sessionDurationMinutes, preferredDays: ptClients.preferredDays }).from(ptClients).where(and(eq(ptClients.ownerProfileId, owner.authUserId), clientId ? eq(ptClients.id, clientId) : and(eq(ptClients.firstName, firstName), eq(ptClients.lastName, lastParts.join(" ") || "Client")))).limit(1);
