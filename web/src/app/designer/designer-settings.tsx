...
export function DesignerSettings({ role, onClose, onOpenTeam, onSaved }: { role: "owner" | "pt"; onClose: () => void; onOpenTeam?: () => void; onSaved: (settings: QualitySettings) => void }) {
  ...
  async function save() {
    ...
    try { const saved = await updateDesignerSettingsAction(settings); setSettings(saved); onSaved(saved); onClose(); }
    catch (saveError) { setError(saveError instanceof Error ? saveError.message : "Settings could not be saved"); }
    finally { setSaving(false); }
  }

  return <div className="modal-backdrop"><section className="settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-heading"><header><div><p className="eyebrow">{role === "owner" ? "WORKSPACE SETTINGS" : "MY PT SETTINGS"}</p><h2 id="settings-heading">Settings</h2><p>{role === "owner" ? "Manage programme quality rules and the practitioners who can access this workspace." : "Manage the quality signals used while reviewing your assigned client programmes."}</p></div><button className="close-button" onClick={onClose}>×</button></header><div className="settings-content"><div className="settings-section settings-role-section"><div className="settings-section-heading"><div><p className="eyebrow">ACCESS</p><h3>{role === "owner" ? "Workspace administration" : "Your assigned workspace"}</h3></div><span>{role === "owner" ? "Owner" : "PT"}</span></div><p className="settings-role-copy">{role === "owner" ? "Invite practitioners, block or reactivate access, and assign each client to the right practitioner." : "Your client list and programme data are limited to clients assigned to you."}</p>{role === "owner" && onOpenTeam && <button type="button" className="outline-button" onClick={onOpenTeam}>Manage team access →</button>}</div><div className="settings-section"><div className="settings-section-heading"><div><p className="eyebrow">PROGRAMME REVIEW</p><h3>Choose the signals you want to see</h3></div><span>{loading ? "Loading…" : "Applies globally"}</span></div>{checks.map(([key, label, description]) => <label className="settings-toggle" key={key}><span><strong>{label}</strong><small>{description}</small></span><input type="checkbox" checked={Boolean(settings[key])} onChange={(event) => setSettings((current) => ({ ...current, [key]: event.target.checked }))} /></label>)}</div><div className="settings-section settings-numbers"><div className="settings-section-heading"><div><p className="eyebrow">THRESHOLDS</p><h3>Make the checks fit your practice</h3></div></div><label>MAX SETS PER SESSION<input type="number" min={1} max={100} value={settings.maxSetsPerSession} onChange={(event) => setSettings((current) => ({ ...current, maxSetsPerSession: Number(event.target.value) }))} /><small>Advisory only; the PT can override a warning.</small></label><label>PRESS / PULL TOLERANCE<input type="number" min={0} max={10} value={settings.pressPullTolerance} onChange={(event) => setSettings((current) => ({ ...current, pressPullTolerance: Number(event.target.value) }))} /><small>How many more pressing exercises than pulling exercises are tolerated.</small></label></div></div>{error && <p className="form-error" role="alert">{error}</p>}<footer><button className="secondary-button" onClick={onClose}>Cancel</button><button className="primary-button" onClick={save} disabled={saving || loading}>{saving ? "Saving settings…" : "Save settings →"}</button></footer></section></div>;
}
