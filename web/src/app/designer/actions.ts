...
import { ptAssessments, ptClients, ptExercisePrescriptions, ptExercises, ptGoals, ptLocations, ptPreferences, ptProgrammeEvents, ptProgrammeWeeks, ptProgrammes, ptSessions } from "@/lib/db/schema";
...
export async function saveProgrammeAction(rawInput: z.input<typeof programmeInputSchema>) {
  ...
  const [week] = await db.insert(ptProgrammeWeeks).values({ programmeId: programme.id, weekNumber: 1, focus: "Foundation / familiarisation", volumeTarget: "Moderate", intensityTarget: "RIR 2–3" }).returning({ id: ptProgrammeWeeks.id });
  if (!week) throw new Error("Programme week could not be saved.");
  const [session] = await db.insert(ptSessions).values({ programmeWeekId: week.id, dayOfWeek: 1, name: "Monday · Strength / Hypertrophy", sessionType: "mixed", durationMinutes: input.sessionDurationMinutes, warmupMinutes: 5 }).returning({ id: ptSessions.id });
  ...
  const exerciseRows = input.exercises.map((exercise, index) => {
    ...
    return match ? { sessionId: session.id, exerciseId: match.id, orderIndex: index, sets: exercise.sets, repsMin: exercise.repsMin ?? null, repsMax: exercise.repsMax ?? null, intensityType: "rir" as const, intensityValue: exercise.intensityValue, restSeconds: exercise.restSeconds ?? null, tempo: exercise.tempo || null, progressionRule: "When all sets reach the top of the range at target RIR with acceptable technique, add a small load increment." } : null;
  }).filter((row): row is NonNullable<typeof row> => row !== null);
  if (exerciseRows.length) await db.insert(ptExercisePrescriptions).values(exerciseRows);
  await db.insert(ptProgrammeEvents).values({ programmeId: programme.id, actorProfileId: owner.authUserId, action: "draft_saved", details: { exerciseCount: exerciseRows.length, version: programme.version } });
  revalidatePath("/designer");
  return { programmeId: programme.id, version: programme.version, exerciseCount: exerciseRows.length };
}
