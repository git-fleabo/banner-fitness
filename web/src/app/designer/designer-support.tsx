...
export function MobileNav({ onClose, onOverview, onClients, onProgrammes, onProgrammeLibrary, onLibrary, onSettings }: { onClose: () => void; onOverview: () => void; onClients: () => void; onProgrammes: () => void; onProgrammeLibrary: () => void; onLibrary: () => void; onSettings: () => void }) {
  return <div className="mobile-nav-backdrop" onClick={onClose}><nav className="mobile-nav-panel" onClick={(event) => event.stopPropagation()}><div className="brand-mark"><img src="/banner-fitness-logo-hex.png" alt="Banner Fitness" /></div><button className="close-button" onClick={onClose}>×</button><button onClick={onOverview}><Icon name="overview" /> Overview</button><button onClick={onClients}><Icon name="clients" /> Clients</button><button onClick={onProgrammes}><Icon name="programmes" /> Programmes</button><button onClick={onProgrammeLibrary}><Icon name="programmes" /> Programme library</button><button onClick={onLibrary}><Icon name="library" /> Exercise library</button><button onClick={onSettings}><Icon name="settings" /> Settings</button><small className="mobile-nav-user">Noaman · Personal trainer</small></nav></div>;
}
