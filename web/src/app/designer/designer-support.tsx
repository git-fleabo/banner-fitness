...
export function MobileNav({ onClose, onOverview, onClients, onLibrary }: { onClose: () => void; onOverview: () => void; onClients: () => void; onLibrary: () => void }) {
  return <div className="mobile-nav-backdrop" onClick={onClose}><nav className="mobile-nav-panel" onClick={(event) => event.stopPropagation()}><div className="brand-mark"><span>A</span><div><strong>ASCEND</strong><small>PT STUDIO</small></div></div><button className="close-button" onClick={onClose}>×</button><button onClick={onOverview}>⌂ Overview</button><button onClick={onClients}>♧ Clients</button><button>▦ Programmes</button><button onClick={onLibrary}>◈ Exercise library</button><small className="mobile-nav-user">Noaman · Personal trainer</small></nav></div>;
}
