...
import { buildEditorSessionState, weekdayLabels, type EditorExercise, type SavedSession } from "@/lib/programme-editor";
...
export function SessionEditorModal({ clientName, goal, days, preferredDays, week, savedSessions, onClose, onSaved, notify }: { clientName: string; goal: string; days: number; preferredDays?: number[]; week: EditorExercise[]; savedSessions?: SavedSession[]; onClose: () => void; onSaved?: () => void; notify: (message: string) => void }) {
  const initialState = buildEditorSessionState({ preferredDays, trainingDays: days, week, savedSessions });
  const [sessionDays] = useState(initialState.days);
  ...
  async function save() {
    ...
    try {
      ...
      await saveProgrammeAction({ clientName, goalSummary: goal, trainingDays: sessionDays.length, sessionDurationMinutes: 45, sessionDays, sessionNames: names, exercises: firstSession.map(toDraftExercise), sessionExercises: Object.fromEntries(sessionDays.map((day) => [String(day), (sessions[String(day)] ?? []).map(toDraftExercise)])) });
