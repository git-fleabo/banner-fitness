...
export function MobileNav({ onClose, onOverview, onClients, onProgrammes, onLibrary, onSettings }: { onClose: () => void; onOverview: () => void; onClients: () => void; onProgrammes: () => void; onLibrary: () => void; onSettings: () => void }) {
  return <div className="mobile-nav-backdrop" onClick={onClose}><nav className="mobile-nav-panel" onClick={(event) => event.stopPropagation()}><div className="brand-mark"><span>E</span><div><strong>ENGINE PT</strong><small>CONSTRUCTION</small></div></div><button className="close-button" onClick={onClose}>×</button><button onClick={onOverview}>⌂ Overview</button><button onClick={onClients}>♧ Clients</button><button onClick={onProgrammes}>▦ Programmes</button><button onClick={onLibrary}>◈ Exercise library</button><button onClick={onSettings}>⚙ Settings</button><small className="mobile-nav-user">Noaman · Personal trainer</small></nav></div>;
}
