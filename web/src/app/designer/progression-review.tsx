...
function ProgressionReview({ clientId, onClose }: { clientId: string; onClose: () => void }) {
  ...
  return <Dialog className="progression-review-modal" titleId="progression-review-heading" onClose={onClose} closeLabel="Close progression review"><header><div><p className="eyebrow">PROGRAMME ADAPTATION</p><h2 id="progression-review-heading">Progression review</h2><p>Rule-based suggestions from the latest logged set data. PT approval is always required; pain, technique and recovery gates take priority over load increases.</p></div></header>{loading ? <LoadingState>Reviewing logged performance…</LoadingState> : error ? <ErrorAlert>{error}</ErrorAlert> : decisions.length === 0 ? <p className="library-empty">No saved programme prescriptions yet.</p> : <div className="progression-review-list">{decisions.map((item) => <article key={item.prescriptionId}><div><strong>{item.exerciseName}</strong><small>{item.pattern} · {item.prescription} · {item.intensityValue}</small></div><span className={`progression-action ${item.decision?.action ?? "pending"}`}>{item.decision?.action ?? "pending"}</span><p>{item.decision ? item.decision.reason : "No logged set result yet. Record a result before changing the next exposure."}{item.decision?.nextLoadKg !== undefined ? ` Suggested next load: ${item.decision.nextLoadKg} kg.` : ""}</p><small>{item.lastDate ? `Latest result ${item.lastDate}` : "No result recorded"}</small></article>)}</div>}<footer><button className="primary-button" onClick={onClose}>Close review</button></footer></Dialog>;
}
