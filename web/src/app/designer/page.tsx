...
import { createClientAction, logWorkoutResultAction, saveProgrammeAction, updateClientAction } from "./actions";
...
type ClientDetail = { client: { id: string; firstName: string; lastName: string; dateOfBirth: string | null; sexOrGender: string | null; sessionDurationMinutes: number | null; preferredDays: unknown; sleepHours: string | null; stressLevel: string | null }; assessment: { clearanceRequired: boolean; riskFlags: unknown; reviewDate: string | null; assessmentDate: string; ptNotes: string | null } | null; goal: { goalType: string; target: string | null; metric: string | null } | null; location: { name: string; locationType: string; equipment: unknown } | null; programme: { id: string; name: string; goalSummary: string; status: string; currentWeek: number; durationWeeks: number; version: number; rationale: string | null; week: { id: string; weekNumber: number; focus: string } | null; sessions: Array<{ id: string; dayOfWeek: number; name: string; sessionType: string; durationMinutes: number; exercises: Exercise[] }> } | null };
  ...
  return (
    <main className="designer-shell">
      ...
      <section className="designer-content">
        ...
        {showLibrary ? <Library onClose={() => { setShowLibrary(false); setActiveNav("Overview"); }} /> : showClients ? <ClientList onClose={() => { setShowClients(false); setActiveNav("Overview"); }} onNew={() => setShowOnboarding(true)} onOpen={(id, name) => { setClientId(id); setClient(name); setShowClients(false); setActiveNav("Overview"); setShowClient(true); }} /> : <>
      ...
      {showClient && clientDetail?.programme && <ProgrammeQualityCard programme={clientDetail.programme} goal={goal} trainingDays={days} />}
...
function ClientWorkspace({name,goal,days,setGoal,setDays,screening,setScreening,expanded,setExpanded,onClose,onEditSessions,notify,week,programme,location,detail,loading}:{name:string;goal:string;days:number;setGoal:(v:string)=>void;setDays:(v:number)=>void;screening:boolean;setScreening:(v:boolean)=>void;expanded:string|null;setExpanded:(v:string|null)=>void;onClose:()=>void;onEditSessions:()=>void;notify:(v:string)=>void;week:Exercise[];programme:ClientDetail["programme"];location:ClientDetail["location"]|undefined;detail:ClientDetail|null;loading:boolean}) {
  const [saving, setSaving] = useState(false);
  const [showWorkoutLog, setShowWorkoutLog] = useState(false);
  async function saveDraft() {
    if (!week.length) { notify("Add exercises before saving the first draft"); return; }
    setSaving(true);
    try {
  ...
  return <div className="workspace-overlay"><div className="workspace-drawer"><header className="workspace-header"><div><p className="eyebrow">CLIENT WORKSPACE</p><h1>{name}</h1><p className="panel-muted">{loading ? "Loading client record…" : `${days} days / week · ${programme ? programme.sessions[0]?.durationMinutes ?? 45 : 45} min · ${location?.name ?? "Location not set"}`}</p></div><button className="close-button" onClick={onClose}>×</button></header><div className="workspace-tabs"><span className="active">Overview</span><span>Assessment</span><span>Programme</span><span>History</span></div><div className="workspace-body"><div className="client-column"><section className="client-hero"><div className="large-avatar">{name.split(" ").map((part) => part[0]).join("").slice(0, 2)}</div><div><span className="status-pill">Active client</span><h2>{goal}</h2><p>{days} days / week · {location?.locationType ?? "Training location not set"}</p></div></section><section className="warning-card"><div className="warning-symbol">!</div><div><strong>{screening ? "Screening review required" : "Screening recorded"}</strong><p>{screening ? "Review the stored screening flags and follow the appropriate referral or clearance process before finalising high-risk work." : "No current screening flags require escalation."}</p></div><button onClick={() => setScreening(!screening)}>{screening ? "View assessment" : "Review"} →</button></section><div className="two-col"><section className="inner-panel"><div className="inner-heading"><h3>Client snapshot</h3><button className="text-button">Edit</button></div><label>PRIMARY GOAL<select value={goal} onChange={e => setGoal(e.target.value)}><option>Fat loss + muscle retention</option><option>General strength</option><option>Hypertrophy</option><option>General fitness</option></select></label><label>TRAINING DAYS<div className="segmented">{[2,3,4].map(day => <button key={day} className={days === day ? "selected" : ""} onClick={() => setDays(day)}>{day} days</button>)}</div></label><div className="mini-facts"><span><b>Sleep</b> {detail?.client.sleepHours ?? "Not recorded"}</span><span><b>Stress</b> {detail?.client.stressLevel ?? "Not recorded"}</span><span><b>Assessment</b> {detail?.assessment?.assessmentDate ?? "Not recorded"}</span><span><b>Location</b> {location?.name ?? "Not recorded"}</span></div></section><section className="inner-panel rationale-box"><div className="inner-heading"><h3>Programming rationale</h3><span className="sparkle">✦</span></div><p>{programme?.rationale ?? "Save a programme draft after selecting exercises and reviewing the client constraints."}</p><button className="outline-button" onClick={() => notify("Rationale copied to notes")}>Copy rationale</button></section></div></div><aside className="plan-column"><div className="plan-header"><div><p className="eyebrow">{programme ? "SAVED PROGRAMME" : "PROGRAMME BUILDER"}</p><h2>{programme ? `${programme.name} / Week ${programme.currentWeek}` : "No programme saved yet"}</h2></div><button className="more-button">•••</button></div>{programme ? <><div className="week-selector"><strong>Week {programme.currentWeek} <small>{programme.week?.focus ?? "Programme week"}</small></strong></div><div className="session-tabs">{programme.sessions.map((session) => <span key={session.id} className={session.dayOfWeek === 1 ? "active" : ""}>{["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"][session.dayOfWeek % 7].slice(0, 3).toUpperCase()} <b>{session.name}</b></span>)}</div><div className="session-title"><div><h3>{programme.sessions[0]?.name ?? "Programme sessions"}</h3><p>{programme.sessions[0]?.durationMinutes ?? 0} min · Week {programme.currentWeek}</p></div><div className="session-actions"><button className="session-editor-launcher" onClick={onEditSessions}>Edit all sessions</button><button className="edit-button" onClick={() => setShowWorkoutLog(true)}>Log result</button></div></div><div className="exercise-list">{week.length ? week.map((exercise,index) => <ExerciseCard key={exercise.name} exercise={exercise} index={index} expanded={expanded === exercise.name} onToggle={() => setExpanded(expanded === exercise.name ? null : exercise.name)} notify={notify}/>) : <div className="dashboard-empty">No prescriptions saved for this session yet.</div>}</div></> : <div className="dashboard-empty">No saved programme is linked to this client yet. Build and save a draft from the programme editor once exercises are selected.</div>}<div className="quality-card"><div className="quality-score">{programme ? "—" : ""}</div><div><strong>Programme quality check</strong><p>{programme ? "Review the rule-based checks before assigning this programme." : "Quality checks will appear after a draft programme is saved."}</p></div></div><div className="plan-footer-actions"><button className="outline-button" onClick={downloadReport}>Download report</button><button className="assign-button" onClick={saveDraft} disabled={saving || !week.length}>{saving ? "Saving draft…" : "Save draft & review →"}</button></div></aside></div></div>{showWorkoutLog && <WorkoutLogModal clientName={name} onClose={() => setShowWorkoutLog(false)} onSaved={() => { setShowWorkoutLog(false); notify("Workout result saved"); }} />}</div>;
}
...
function ProgrammeQualityCard({programme,goal,trainingDays}:{programme:NonNullable<ClientDetail["programme"]>;goal:string;trainingDays:number}) { const sessions = programme.sessions; const allExercises = sessions.flatMap((session) => session.exercises); const totalSets = allExercises.reduce((sum, exercise) => sum + Number(exercise.prescription.match(/^\d+/)?.[0] ?? 0), 0); const presses = allExercises.filter((exercise) => /push|press/i.test(exercise.pattern)).length; const pulls = allExercises.filter((exercise) => /pull|row/i.test(exercise.pattern)).length; const missingProgression = allExercises.filter((exercise) => !exercise.progressionRule).length; const warnings = [sessions.length < trainingDays ? `Only ${sessions.length} session${sessions.length === 1 ? "" : "s"} saved for the selected ${trainingDays}-day target.` : "", presses > pulls + 2 ? "Pressing volume is high relative to pulling volume." : "", totalSets > sessions.length * 28 ? "Total prescribed sets may create a high recovery demand." : "", missingProgression ? `${missingProgression} exercise${missingProgression === 1 ? " lacks" : "s lack"} a documented progression rule.` : "", sessions.some((session) => { const sets = session.exercises.reduce((sum, exercise) => sum + Number(exercise.prescription.match(/^\d+/)?.[0] ?? 0), 0); return sets * 2 + 5 > session.durationMinutes; }) ? "At least one session may run over its saved duration at the current prescription." : ""].filter(Boolean); const score = Math.max(0, 100 - warnings.length * 8); return <section className="quality-summary" aria-label="Live programme quality summary"><div className="quality-summary-heading"><div><p className="eyebrow">LIVE QUALITY CHECK</p><strong>Advisory score</strong></div><span className="quality-summary-score">{score}</span></div><p className="quality-summary-meta">{totalSets} total sets · {sessions.length} sessions · {goal}</p>{warnings.length ? <ul>{warnings.slice(0, 3).map((warning) => <li key={warning}>! {warning}</li>)}</ul> : <p className="quality-summary-good">No rule-based concerns detected from the saved prescription.</p>}<small>This is a decision-support signal, not an objective programme-quality measure. PT review remains required.</small></section>; }
