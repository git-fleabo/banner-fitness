...
const equipmentFilters = [{ value: "Barbell", label: "Barbell" }, { value: "Dumbbells", label: "Dumbbells" }, { value: "Machines", label: "Machines" }, { value: "Cable", label: "Cable" }, { value: "Bodyweight", label: "Bodyweight" }, { value: "Kettlebell", label: "Kettlebell" }, { value: "TRX", label: "TRX" }, { value: "Gymnastic rings", label: "Rings" }, { value: "Resistance band", label: "Bands" }, { value: "Running", label: "Running" }, { value: "Treadmill", label: "Treadmill" }, { value: "Bike", label: "Bike" }, { value: "Rowing machine", label: "Rowing machine" }, { value: "Cross trainer", label: "Cross trainer" }];
const filterPresets = [
...
export function ProgrammeLibrary({ clients, onClose, onApply, notify }: { clients: ProgrammeLibraryClient[]; onClose: () => void; onApply: (template: ProgrammeTemplate, client: ProgrammeLibraryClient) => void; notify: (message: string) => void }) {
  ...
  const filtered = [...filterProgrammeLibraryTemplates(templates, { query, goal: goalFilter, frequency: frequencyFilter === "all" ? "all" : Number(frequencyFilter), equipment: equipmentFilter, experienceLevel: experienceFilter, frameworkType: frameworkFilter, difficultyLevel: difficultyFilter === "all" ? "all" : Number(difficultyFilter) as 1 | 2 | 3 })].sort((a, b) => (a.variantGroup ?? a.goal).localeCompare(b.variantGroup ?? b.goal) || (a.difficultyLevel ?? 1) - (b.difficultyLevel ?? 1) || a.label.localeCompare(b.label));
  ...
  return <div className="programme-library-view">
    ...
    <div className="programme-level-legend" aria-label="Programme difficulty levels"><span>DIFFICULTY</span><span><b>1</b>Foundation</span><span><b>2</b>Progression</span><span><b>3</b>Experienced</span></div>
    {error && <p className="form-error programme-library-error" role="alert">{error}</p>}
    {loading ? <p className="library-empty">Loading the programme library…</p> : filtered.length ? <div className="programme-library-grid">{filtered.map((template) => { const level = template.difficultyLevel ?? 1; return <article className={`programme-library-card level-${level}`} key={template.id}><div className="programme-library-card-heading"><div><p className="eyebrow">{template.goal}</p><h2>{template.label}</h2></div><span className={`programme-difficulty level-${level}`}>Level {level} · {level === 3 ? "Experienced" : level === 2 ? "Progression" : "Foundation"}</span></div><p>{template.description || "Reusable starting point for PT adaptation."}</p><div className="programme-library-meta"><span>{template.sessionDurationMinutes} min</span><span>{template.sessions.length} sessions</span><span>{template.sessions.reduce((sum, session) => sum + session.exercises.length, 0)} exercises</span><span>{template.experienceLevel ?? "Varied"}</span><span>{template.frameworkType ?? "Custom"}</span></div><div className="programme-library-actions"><button className="primary-button" onClick={() => { setApplySource(template); setApplyClientId(clients[0]?.id ?? ""); }}>Apply to client →</button><button className="secondary-button" onClick={() => setPreview(template)}>Preview →</button><button className="secondary-button" onClick={() => { setDuplicateSource(template); setDuplicateName(`${template.label} copy`); }}>Duplicate</button><button className="secondary-button" onClick={() => setEditing(template)}>Edit →</button></div></article>; })}</div> : <div className="empty-client-state"><h3>No programme templates match</h3><p>Try another combination of goal, frequency, equipment, difficulty or framework.</p><button className="secondary-button" onClick={resetFilters}>Reset filters</button></div>}
