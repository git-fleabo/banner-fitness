...
export async function requireWorkspaceOwner(): Promise<ActiveOwner | NextResponse> {
  const access = await getAccountAccess();
  if (access.state === "unauthenticated") return NextResponse.json({ error: OWNER_ACCESS_ERROR }, { status: 401 });
  if (access.state !== "active" || access.account.role !== "owner") return NextResponse.json({ error: "Workspace owner access required" }, { status: 403 });
  return access.account;
}
