...
describe("client timeline", () => {
  it("combines client context, programming and progress records in newest-first order", () => {
    const timeline = buildClientTimeline({
      ...
      qualityReviews: [{ id: "quality-1", programmeId: "programme-1", programmeName: "Strength foundation", score: 82, approvalReadiness: "needs_review", blockingCount: 0, significantCount: 1, advisoryCount: 2, evidenceVersion: "2026", evaluatedAt: "2026-01-08T09:00:00.000Z" }],
    ...
    expect(timeline.map((item) => item.kind)).toEqual(expect.arrayContaining(["profile", "assessment", "goal", "location", "preferences", "programme", "quality", "event", "workout", "performance"]));
  ...
  it("makes screening concerns and workout pain visible without diagnosing", () => {
    const timeline = buildClientTimeline({
      ...
      goals: [], locations: [], preferences: [], programmes: [], qualityReviews: [], programmeEvents: [],
